...
func (p *PBFT) HandleMessage(msg *pb.ConsensusMessage) {
	...
	case "PREPARE":
		...
		fmt.Printf("[Collector %s] -> Aggregated PREPARE weight: %d/%d\n", p.ID, p.PrepareVotes[k], p.quorum())
	...
	case "COMMIT":
		...
		fmt.Printf("[Collector %s] -> Aggregated COMMIT weight: %d/%d\n", p.ID, p.CommitVotes[k], p.quorum())
